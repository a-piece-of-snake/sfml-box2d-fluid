...
void ParticleGroup::UpdateData(World world) {
    ...
    float ParticleGroup::GetForce(float dst, float radius) {
        ...
        float x = radius - dst;
        float x_sq = x * x;
        float x_pow2_5 = x_sq * MathUtils::Q_sqrt(x);
		float radius_sq = radius * radius;
        return x_pow2_5 / (B2_PI * radius_sq * radius_sq / 4.0f);
    }
    ...
    */
    ...
    void ParticleGroup::ComputeChunkForces(int start, int end, int threadID, float timestep) {
        ...
        thread_local float density = 0.0f;
        thread_local b2Vec2 neighborPosSum = b2Vec2_zero;
        thread_local b2Vec2 neighborVecSum = b2Vec2_zero;
        thread_local int attractCount = 0;
        thread_local b2Vec2 velA = b2Vec2_zero;
        thread_local float radiusA = Config.Impact;
        thread_local float radiusB = Config.Impact;
        thread_local float dst = 0.f;
        ...
        for (int idx = start; idx < end; ++idx) {
            ...
            if (b2Body_IsAwake(p.bodyId)) {
                ...
                register b2Vec2 posA = b2Vec2_zero;
                register b2Vec2 posB = b2Vec2_zero;
                register b2Vec2 offset = b2Vec2_zero;
                neighborPosSum = b2Vec2_zero;
                neighborVecSum = b2Vec2_zero;
                attractCount = 0;
                posA = p.pos;
                posB = b2Vec2_zero;
                offset = b2Vec2_zero;
                velA = p.LinearVelocity;
                radiusA = Config.Impact;
                ...
                for (Particle* other : neighbors) {
                    ...
                    offset = posB - posA;
                    float offsetLength = MathUtils::b2Length(offset);
                    if (offsetLength < 0.001f) continue;

                    dst = offsetLength / (p.shape.radius / Config.Impact);
                    float effectiveRange = (radiusA + radiusB) * Config.Impact;
                    if (dst < effectiveRange) {
                        b2Vec2 forceDir = b2Normalize(offset);

                        float densityScale = std::clamp(density, Config.MIN_DENSITY, Config.MAX_DENSITY);
                        float distanceForceMag = std::min(GetForce(dst, effectiveRange), Config.MAX_FORCE) * densityScale* densityScale * MathUtils::Q_sqrt(MathUtils::Q_sqrt(densityScale));
                        ...
                        float len = MathUtils::b2Length(velDiff);
                        if (len > maxVelDiff) {
                        ...
                        totalForce.y = std::clamp(totalForce.y, -Config.MAX_GET_FORCE, Config.MAX_GET_FORCE);
    ...
    void ParticleGroup::ComputeParticleForces(float timestep) {
        ...
        for (int start = 0; start < n; start += chunkSize) {
            ...
            futures.push_back(
                pool.enqueue([this, start, end, threadCounter, timestep]() {
                    ComputeChunkForces(start, end, threadCounter, timestep);
                    })
            );
            threadCounter++;
        }
